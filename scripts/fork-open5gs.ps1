###############################################################################
# Open5GS Fork Script for Distributed EPC (PowerShell Version)
...
$BackupDir = "C:\temp\open5gs-backup-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
...
if (Test-Path $WorkDir) {
    if ($Force) {
        ...
        Write-Host "❌ Work directory already exists. Use -Force to overwrite." -ForegroundColor Red
        exit 1
    }
}
...
$SourceDocs = "$PSScriptRoot\..\docs\distributed-epc\*.md"
...
Write-Host ""
Write-Host "🔧 To continue development:" -ForegroundColor Blue
Write-Host "cd $WorkDir\open5gs-distributed"
Write-Host "git checkout distributed-epc"
Write-Host ""
