<script lang="ts">
  ...
  // Simplified settings - only PFX cert and enhanced features
  let formData = {
    // PFX Certificate
    pfxCertificate: config.pfxCertificate || '',
    pfxPassword: config.pfxPassword || '',
    pfxCertificateName: config.pfxCertificateName || '',
  ...
  async function handlePFXUpload(event: Event) {
    const target = event.target as HTMLInputElement;
    const file = target.files?.[0];
    if (file) {
      pfxFile = file;
      formData.pfxCertificateName = file.name;
      
      // Read file as base64
      const reader = new FileReader();
      reader.onload = (e) => {
        const result = e.target?.result as string;
        formData.pfxCertificate = result.split(',')[1]; // Get base64 part
      };
        ...
        <form on:submit|preventDefault={handleSave}>
          ...
          <div class="info-banner">
            ...
            <div class="info-content">
              <strong>Optional Security Configuration</strong>
              <p>Upload a PFX certificate for enhanced mTLS security with Google SAS. This is optional but recommended for production deployments.</p>
            </div>
          </div>
          
          <!-- PFX Certificate Section -->
          <div class="form-section">
            <h4>🔒 Client Certificate (Optional)</h4>
            
            <div class="form-group">
              <label>PFX Certificate (.pfx or .p12)</label>
              <input 
                type="file" 
                accept=".pfx,.p12"
                on:change={handlePFXUpload}
                class="file-input"
              />
              {#if formData.pfxCertificateName}
                <span class="file-status">✅ {formData.pfxCertificateName}</span>
              {/if}
              <span class="form-hint">
                PFX/PKCS#12 file containing both certificate and private key
              </span>
            </div>
            ...
            <div class="form-group">
              ...
              <input 
                type="password" 
                bind:value={formData.pfxPassword}
                placeholder="Enter PFX password"
                autocomplete="off"
              />
              <span class="form-hint">
                Password to decrypt the PFX certificate file
              </span>
            </div>
          </div>
          ...
          <div class="security-notice">
            ...
            <div class="notice-content">
              <strong>Security:</strong> All certificates and passwords are encrypted and stored securely in Firestore.
              Your configuration is isolated to your tenant account.
            </div>
          </div>
