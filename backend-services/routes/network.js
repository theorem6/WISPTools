...
router.get('/hardware-deployments', async (req, res) => {
  try {
    ...
    debug.log('[Network API] Fetching hardware deployments with query:', JSON.stringify(query));
    ...
    debug.log('[Network API] Found', hardware.length, 'hardware deployments for tenant:', req.tenantId);
    if (hardware.length > 0) {
      debug.log('[Network API] Sample hardware deployment:', {
        id: hardware[0]._id,
      ...
      debug.log('[Network API] Debug info:', {
