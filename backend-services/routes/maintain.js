...
try {
  // Try server structure first (relative to routes/)
  const workOrderSchema = require('../work-order-schema');
  const customerSchema = require('../customer-schema');
  WorkOrder = workOrderSchema.WorkOrder;
  ...
  try {
    ...
    console.error('Could not load models:', e2);
    throw e2;
  }
}
...
router.get('/dashboard/activity', async (req, res) => {
  try {
    ...
    const recentCustomers = await Customer.find({ tenantId })
      .sort({ updatedAt: -1 })
      .limit(limit)
      .lean();
