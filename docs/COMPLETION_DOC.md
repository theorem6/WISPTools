...
| **Backend deploy** | Ran `deploy-backend-to-gce.ps1 -DeployMethod Upload`: upload succeeded; remote SSH failed (plink on Windows). `DEPLOY_BACKEND_FALLBACK.md` updated: run Option A from Cloud Shell to finish (move /tmp/backend-services-deploy → target, npm install, pm2 reload) |
